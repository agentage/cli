...
describe('update lock', () => {
  ...
  const raceProcesses = async (count: number, now: number): Promise<string[]> => {
    ...
    mkdirSync(join(libDir, 'fs'), { recursive: true });
    mkdirSync(join(libDir, 'update'), { recursive: true });
    const sources: [string, string][] = [
      [join(import.meta.dirname, '..', 'fs', 'config.ts'), join(libDir, 'fs', 'config.js')],
      [join(import.meta.dirname, '..', 'fs', 'file-lock.ts'), join(libDir, 'fs', 'file-lock.js')],
      [join(import.meta.dirname, 'update-lock.ts'), join(libDir, 'update', 'update-lock.js')],
    ];
    for (const [src, dest] of sources) {
      const out = ts.transpileModule(readFileSync(src, 'utf-8'), {
        compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 },
      }).outputText;
      writeFileSync(dest, out, 'utf-8');
    }
    const script = join(dir, 'race.mjs');
    const mod = pathToFileURL(join(libDir, 'update', 'update-lock.js')).href;
    writeFileSync(
      script,
