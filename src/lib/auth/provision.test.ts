...
describe('provisionAccountVault', () => {
  it('POSTs {name} to <api>/memories when signed in', async () => {
    const { deps, post } = makeDeps();
    await provisionAccountVault('acct', deps);
    expect(post).toHaveBeenCalledTimes(1);
    expect(post).toHaveBeenCalledWith(auth, target, `${target.api}/memories`, { name: 'acct' });
  });
  ...
  it('200 -> exists (idempotent, the memory is already in the account)', async () => {
  ...
  it('any other status stays non-fatal (offline), keeping the local entry', async () => {
    for (const status of [403, 409, 500]) {
      const post = vi.fn(async () => jsonResponse(status));
      const { deps } = makeDeps({ post });
      const res = await provisionAccountVault('acct', deps);
      expect(res.status).toBe('offline');
